...
function maintop($title,$showtop = true) {
  ...
  if ($showtop) {
    echo "<tr><td><font size=\"2\">\n"
		...
		."<img src=pixel.gif width=7 height=1><a href=\"javascript:history.go(-1);\" $iftop>后退</a>\n"
		."<img src=pixel.gif width=7 height=1><a href=\"javascript:location.reload();\" $iftop>刷新</a>\n"
		."<img src=pixel.gif width=7 height=1><a href=\"javascript:history.go(1);\" $iftop>前进</a>\n"
  ...
  while($stylesheet = readdir($style)) {
    ...
    if ($stylesheet[0] != "." && $stylesheet[0] != ".." ) {
      if (is_dir($folder.$stylesheet) && is_readable($folder.$stylesheet)) { 
        ...
        $content2[$b] ="<td><a target='_blank' href=\"".$folderx.$stylesheet."\">".$sstylesheet."</a></td>\n"
...
function xcop($file, $ndir, $folder) {
  ...
  if (!$file == "") {
    maintop("复制");
    if (copy($folder.$file, $ndir.$file)) {
      echo $folder.$file." 已经成功复制到 ".$ndir.$file
    ."&nbsp;<a href=\"".$adminfile."?op=home\">返回文件管理</a>\n";
    } else {
      echo "无法复制 ".$folder.$file
    ."&nbsp;<a href=\"".$adminfile."?op=home\">返回文件管理</a>\n";
    }
    mainbottom();
  } else {
...
function viewframe($file) {
  ...
  if ($filefolder == "/") {
    ...
    echo "<iframe width=\"99%\" height=\"60%\" src=\"".$folder.$file."\">\n"
...
function mainbottom() {
  echo "</table></table>\n"
      ."<table width=100%><tr><td align=right><font class=copyright>Powered By <a target='_blank' href=http://www.aesthetic.asia/>唯美诗意</a> & <a target='_blank' href=http://www.godwolfs.asia/>天狼星の破晓</a></font></table>\n"
      ."</table></table></body>\n"
      ."</html>\n";
  exit;
}
...
//die($dename);
  //global $dename;
    ...
    echo "<table border=\"0\" cellpadding=\"2\" cellspacing=\"0\">\n"
        ...
        ."确定要进行目录压缩?<br><br>\n"
        ."<a href=\"".$adminfile."?op=zip&dename=".$dename."&folder=".$_REQUEST['folder']."\">确定</a> | \n"
        ."<a href=\"".$adminfile."?op=home\"> 取消 </a>\n"
        ."</table>\n";
    mainbottom();
}
